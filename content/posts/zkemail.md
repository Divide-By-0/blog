...
Centralized attestation to the DNS key rotation gives unilateral power to the attester over the protocol. If we outsourced to the client or a server, we'd still have to somehow ensure that the DNS record wasn't being spoofed at the moment the client was verifying the proof. A few clever solutions combining self-custody key rotations, governance methods, and fraud proofs [^2] allow a fairly robust mechanism to handle this process. Ideally though, we can lobby websites to enable DNSSEC :).
