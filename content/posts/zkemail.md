...
To maximize user privacy, we ideally want to generate proofs on the client side so users don't have to send their email contents to an external server. But there are two other properties that trade off against privacy: speed and compression. We'd like to be able to generate proofs fast; make them small enough to efficiently verify on-chain; and do as much of the proving locally as is practical. Different users value these properties to different degrees, so our solution is to make different compromises depending on the user's preferences.

For the most privacy-conscious users, we can allow client-side proving. But to maximize user-friendliness, we can default to a permissionless network of relayers that generate proofs on the user's behalf. In this case, you have to trust your relayer, but in exchange you get speed: since the Circom circuits range from one million constraints without body verification to 3-8+ million with body verification, these proofs can take minutes and several CPUs on a user's browser. The same proving on 128-core machines can take less than five seconds.
