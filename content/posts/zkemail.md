...
To maximize user privacy, we'd ideally like to generate proofs on the client side so users don't have to send the contents of their emails to an external server. But there are two other properties that trade off against privacy: speed and compression. We'd like to be able to generate proofs fast; make them small enough to efficiently verify on-chain; and do as much of the proving locally as is practical. Different users value these properties to different degrees, so our solution is to make different compromises depending on the user's preferences.
